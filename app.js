...
function migrateWatches(ws){
 const raw=Array.isArray(ws)&&ws.length?ws:defaults;
 const out=raw.map(w=>({...w,active:w.active!==false,subs:Array.isArray(w.subs)?w.subs.slice(0,20):[],spaces:Array.isArray(w.spaces)&&w.spaces.length?w.spaces:(String(w.name).toLowerCase()==='paie'?['work']:['private'])}));
 // Migration V16 finale : garantit les 3 veilles de base même après un ancien localStorage incomplet.
 for(const d of defaults){if(!out.some(w=>String(w.name).toLowerCase()===d.name.toLowerCase()))out.push(structuredClone(d));}
 return out;
}
...
function morePage(){return topItems('☰ Plus','Tout le reste à portée de bec.','assets/emu-breakfast.png')+`<div class="menu-list"><button data-go="web">🌐 <span><b>Recherche Web</b><small>Thèmes, sous-thèmes et sources actives</small></span>›</button><button data-go="sources">☷ <span><b>Mes sources</b><small>Ajouter, modifier, tester ou désactiver</small></span>›</button><button data-go="settings">⚙️ <span><b>Paramètres</b><small>Comptes Google, veilles et sécurité</small></span>›</button></div><div class="version-stamp">PointJour — version finale 2026.09</div>`}
